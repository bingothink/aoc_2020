...
def Part1():
    p1 = list(map(int,inp[0].split('\n')[1:]))
    p2 = list(map(int,inp[1].split('\n')[1:-1]))
    while len(p1) > 0 and len(p2) > 0:
        p1, p2 = playCombat(p1, p2)
    if len(p1) > 0:
        return getScore(p1)
    else:
        return getScore(p2)
...
# PART 2 #
##########
def currentHash(p1,p2):
    return hash((tuple(p1), tuple(p2)))

def playRecursiveCombat(p1, p2):
    hashes = []
    while len(p1) > 0 and len(p2) > 0:
        if currentHash(p1,p2) in hashes:
            # player 1 wins the game automatically
            p1.append(p1[0])
            p1.extend(p2)
            del p1[0]
            return p1,[]
        else:
            hashes.append(currentHash(p1,p2))
            if len(p1) > p1[0] and len(p2) > p2[0]:
                r1,r2 = playRecursiveCombat(p1[1:p1[0]+1], p2[1:p2[0]+1])
                # player two hand wins
                if len(r2) >0:
                    p2.append(p2[0])
                    p2.append(p1[0])
...
def Part2():
    p1 = list(map(int,inp[0].split('\n')[1:]))
    p2 = list(map(int,inp[1].split('\n')[1:-1]))
    r1, r2 = playRecursiveCombat(p1, p2)
    if len(r1) > 0:
        return getScore(r1)
    else:
        return getScore(r2)
