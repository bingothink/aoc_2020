...
def Part1():
    # depart time is first line
    depart = int(inp[0])
    # split bus schedule by comma and remove duplicates
    schedule = list(set(inp[1].split(',')))
    ...
    bestBus = (0,9999999)
    for bus in schedule: 
        # departs right on time
        if depart % bus == 0:
            time = depart
        else:
            # find when this departs after our depart time
            time = depart - (depart % bus) + bus
        # this is the best time
        if time < bestBus[1]:
            bestBus = (bus,time)
    # return wait time delta * bus number
    return (bestBus[1]-depart) * bestBus[0]

def Part2():
    schedule = list(inp[1].split(','))
    busses = []
    for i,b in enumerate(schedule):
        if b != 'x':
            busses.append((int(b),i))
    # busses format: [(busID,t_offset), ...]
    #sort by bus ID
    busses.sort()
    # start at the beginning and increment timestamp by 1
    time = 0
    multiplier = 1
